...
export default function Contact() {
  ...
  <div className="page contact-page">
    <motion.section className="page-hero grid-paper reveal revealed" initial={{opacity: 0, y: 30}} whileInView={{opacity: 1, y: 0}} viewport={{once: true}} transition={{type: "spring", stiffness: 120, damping: 14, mass: 0.8}}>
      <div className="phase-chip"><span>05</span> Delivering the outcome</div>
      <h1>Let’s make<br />something clearer.</h1>
      <p>The final phase of this portfolio. Maybe the first phase of something else.</p>
      <span className="scribble" aria-hidden="true">look closer ↓</span>
    </motion.section>
    
    <motion.section className="section contact-layout reveal revealed" initial={{opacity: 0, y: 30}} whileInView={{opacity: 1, y: 0}} viewport={{once: true}} transition={{type: "spring", stiffness: 120, damping: 14, mass: 0.8}}>
      <div className="contact-side">
        <p className="hand-note">Good projects usually begin with an honest conversation.</p>
        <h2>Tell me what you’re<br />trying to untangle.</h2>
        <p>Whether it’s a product challenge, a leadership question, or a role that feels unusually well matched, I’d like to hear about it.</p>
        <div className="contact-details"><a href="mailto:YOUR_EMAIL">YOUR_EMAIL</a><span></span><span>YOUR_LOCATION</span></div>
        
        <div style={{"marginTop":"3rem"}}>
          <button id="toggle-feedback-btn" className="feedback-toggle-btn">
            Share Feedback about Me 
      ...
      <div className="forms-container">
        <form className="brief-form" id="brief-form">
          <div className="form-title"><span>PROJECT BRIEF</span><small>No. <b>001</b></small></div>
          <label><span>01</span><div>Your name<input name="name" required="" placeholder="What should I call you?" /></div></label>
          <label><span>02</span><div>Email address<input name="email" type="email" required="" placeholder="you@company.com" /></div></label>
          <label><span>03</span><div>What are we talking about?<select name="type" required=""><option value="">Choose a project type</option><option>Product design</option><option>UX leadership</option><option>Research &amp; strategy</option><option>Speaking / collaboration</option><option>Something else</option></select></div></label>
          <div className="form-row"><label><span>04</span><div>Timeline<select name="timeline"><option>Just exploring</option><option>Within a month</option><option>1 to 3 months</option><option>3+ months</option></select></div></label><label><span>05</span><div>Budget<select name="budget"><option>Let’s discuss</option><option>Under ₹1L</option><option>₹1L to ₹3L</option><option>₹3L+</option></select></div></label></div>
          <label><span>06</span><div>The messy context<textarea name="message" required="" placeholder="What’s happening, what have you tried, and what would a good outcome feel like?"></textarea></div></label>
          <button type="submit" className="button">Send brief <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h13M13 6l6 6-6 6" /></svg></button>
          <p className="privacy-note">Your information stays between us. No mailing lists, no awkward follow-ups.</p>
          <div className="success-note sticky-yellow" id="success-note"><div className="success-pin"></div><strong>Brief received!</strong><span>Thanks. I'll be in touch soon.</span></div>
        </form>

        <form className="brief-form" id="feedback-form" style={{"display":"none"}}>
          <input type="hidden" name="type" value="Feedback" />
          <div className="form-title"><span>SHARE FEEDBACK</span><small style={{"cursor":"pointer","textDecoration":"underline"}} id="copy-feedback-link"><b>Copy Link</b></small></div>
          <label><span>01</span><div>Your name<input name="name" placeholder="What should I call you? (Optional)" /></div></label>
          <label><span>02</span><div>Email address<input name="email" type="email" placeholder="you@company.com (Optional)" /></div></label>
          <label><span>03</span><div>Context / Relationship<select name="timeline" required=""><option value="">How do we know each other?</option><option>Co-worker</option><option>Manager</option><option>Client / Partner</option><option>Friend</option><option>LinkedIn</option><option>Other</option></select></div></label>
          <label><span>04</span><div>Feedback<textarea name="message" required="" placeholder="What are your thoughts on my work, communication, or anything else? Be honest!"></textarea></div></label>
          <div style={{"display":"flex","alignItems":"center","gap":"12px","padding":"25px 0","borderBottom":"1px solid var(--line)"}}>
            <input type="checkbox" name="testimonial" id="testimonial" style={{"width":"auto","margin":"0","padding":"0","display":"inline-block","border":"none","boxShadow":"none","appearance":"auto"}} />
            <label htmlFor="testimonial" style={{"display":"inline","padding":"0","border":"none","font":"400 16px var(--body)","color":"var(--ink)","cursor":"pointer","textTransform":"none","letterSpacing":"0"}}>Can you use your feedback as testimonial in my site?</label>
          </div>
          <button type="submit" className="button" style={{"marginTop":"25px"}}>Send Feedback <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h13M13 6l6 6-6 6" /></svg></button>
          <div className="success-note sticky-yellow" id="feedback-success-note"><div className="success-pin"></div><strong>Feedback received!</strong><span>Thanks for sharing.</span></div>
        </form>
      </div>
    </motion.section>
    ...
    <footer className="site-footer">
      ...
      <div className="footer-meta">
        ...
        <span className="footer-views">Views: <strong id="visitor-count" style={{"color":"var(--accent)","fontWeight":"600"}}>1,134</strong></span>
